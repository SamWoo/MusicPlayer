...
public class RandomPlayActivity extends AppCompatActivity {
    ...
    private static final String TAG = RandomPlayActivity.class.getName();
    private List<MusicBean> mList;
    Messenger mMessengerClient;
    private RandomPlayAdapter mAdapter;
...
//        bindService(new Intent(this, MediaPlayerService.class), mServiceConnection, BIND_AUTO_CREATE);
        ...
        mAdapter = new RandomPlayAdapter(this);
//        mAdapter.setOnItemClickListener((view,position)->playSong(position));
        ...
        mList = MyApplication.getDaoSession().getMusicBeanDao().queryBuilder()
                .where(MusicBeanDao.Properties.Type.eq(Constant.MUSIC_HOT)).list();
        if (null != mList) {
